      ...
      $("#mainPage").on("click", "#dv_addr", function() {
          ...
          window.addEventListener('message', function(event) {
            ...
            if (loc && loc.module == 'locationPicker') {//防止其他应用也会向该页面post信息，需判断module是否为'locationPicker'
              ...
              $("#dv_addrdetail").removeClass("gray");
              $("#dv_addrdetail").text(loc.poiaddress);
              $("#mainPage").show();
              $("#mapPage").hide();
            }                                
          }, false); 
  ...
  function renderShopInfo(){
    ...
    if(token&&token.length>0&&registerMoible&&registerMoible.length>0){
      ...
      if(strInfo){
        ...
        shopLat=tempInfo.addressLat;
        shopLng=tempInfo.addressLng;
       ...
       $.ajax({
         ...
         success: function(res2) {
             if (res2.status == 0) {
                ...
                shopLat=res2.data.Latitude;
                shopLng=res2.data.Longitude;
  ...
  function SaveBusiness(){
    ...
    var bDate=nowTime.toString().formatDate("yyyy-MM-dd")
 ...
 function validateInfoState(){
    ...
    returnObj.data.addressLat=shopLat;
    returnObj.data.addressLng=shopLng;
  ...
  function SubmitShop(submitType){
    ...
    $.ajax({
        ...
        data: {"MobileNum":tempObj.data.registerMoible,"Name":tempObj.data.shopName,"TYPE":tempObj.data.shopType,
          ...
          ,"InfoFillState":fillState,"Latitude":tempObj.data.addressLat,"Longitude":tempObj.data.addressLng},
        success: function (res) {
            if(submitType==0){
